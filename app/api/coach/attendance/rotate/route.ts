...
async function getUserFromCookiesOrAuthHeader() {
  ...
  const { data: aData } = await admin.auth.getUser(token)
  if (!aData?.user) return { user: null, mode: 'auth_header_invalid' as const }
...
export async function POST(
  req: Request,
  ctx: { params: Promise<{ id: string }> } // en algunas versiones de Next hay que await
) {
  try {
    ...
    if (!user) {
      return NextResponse.json({ error: 'unauthorized', detail: `no_user_${mode}` }, { status: 401 })
    }

    // (opcional) valida que user sea coach o tenga permisos
    ...
    if (error || !data) {
      return NextResponse.json({ error: 'rotate_failed', detail: error?.message }, { status: 500 })
    }
